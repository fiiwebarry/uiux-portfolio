...
import { LANDINGPAGE_PRODUCT_SCHEMA } from "../../utils/constants";

const Inspired = () => {
  return (
    <section className="md:py-20 py-10">
      <div className="grid py-10 md:max-w-7xl md:mx-auto gap-5 justify-items-center md:p-0 p-3">
        <p className="  border border-[#EBF2FF] font-playfair text-xl  py-3 px-6 rounded-3xl bg-[#EBF2FF] text-[#1B1EE4]">
          {" "}
          More Hand-picked highlights
        </p>
        <p className="text-center font-playfair text-[#1F1F1F] md:w-1/2 md:text-6xl">
          Get inspired
        </p>
        <p className="text-center font-gillsans md:mt-2 md:w-9/12 md:text-2xl text-[#1F2937]">
      ...
      <div className="grid md:grid-cols-2 gap-6 md:gap-14 mt-8 md:max-w-7xl md:mx-auto">
        {LANDINGPAGE_PRODUCT_SCHEMA.map((r, i) => {
          return (
            <Link
              key={i}
              to={r.href}
      ...
      <div className="grid justify-items-center md:mt-10 md:mb-4">
        <Link
          to="/casestudies"
